...
namespace app.tasks.startup
{
  public class Startup
  {
    ...
    static void wire_everything_up()
    {
		...
		dependencies.Add(typeof(MatchBuilderFactory), new SimpleFactory(() => (MatchBuilderFactory)Container.fetch.an<IBuildRequestMatchers>));

		dependencies.Add(typeof(IFindPathsToViews), new SimpleFactory(Stub.with<StubPathRegistry>));

		dependencies.Add(typeof(ICreateAResponse), new SimpleFactory(() => new PageFactory(Container.fetch.an<IFindPathsToViews>(), BuildManager.CreateInstanceFromVirtualPath)));

		dependencies.Add(typeof(IDisplayReportModels), new SimpleFactory(() => new WebResponseEngine(Container.fetch.an<ICreateAResponse>(), () => HttpContext.Current)));

		dependencies.Add(typeof(IFindCommands), new SimpleFactory(() => new CommandRegistry(Stub.with<StubSetOfCommands>(), Stub.with<StubMissingCommand>())));
